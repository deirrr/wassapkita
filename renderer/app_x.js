    ...
    async doExport() {
      ...
      try {
        ...
        if (res?.cancelled) {
          ...
          this.statusText = res?.error || "Gagal export kontak.";
        }
      } catch (e) {
...
// =====================
// 2) BLAST PAGE (UPLOAD -> TEMPLATE -> SETTING -> SEND)
// =====================
const BlastView = {
  name: "BlastView",
  data() {
    ...
    if (window.wassapkita?.onBlastProgress) {
      window.wassapkita.onBlastProgress((p) => {
        ...
        if (p.type === "item" && p.item) {
          this.progressText = p.message || this.progressText;
          this.logs.unshift(p.item); // terbaru di atas
        }
      });
    }
  ...
  methods: {
    ...
    async startBlast() {
      ...
      try {
        // IMPORTANT: buang Proxy Vue -> jadikan plain object
      ...
      <div class="blast-step-content">
        ...
        <div v-else-if="step === 'template'" class="blast-panel">
          ...
          <div style="margin-top:12px;">
            <textarea
              ...
              rows="6"
              style="width:100%; resize:vertical; border-radius:12px; padding:12px; border:1px solid rgba(255,255,255,0.10); background: rgba(2,6,23,0.6); color:#e5e7eb;"
              placeholder="Tulis pesan di sini..."
            ></textarea>
          </div>

          <div v-if="templateError" class="blast-subtitle" style="margin-top:10px; color:#fca5a5;">
            {{ templateError }}
          </div>

          <div style="margin-top:12px; display:flex; justify-content:space-between; gap:10px;">
            <button type="button" class="btn" @click="setStep('upload')">Back</button>
            <button type="button" class="btn-primary" @click="goToSetting">Next</button>
          </div>
        </div>
        ...
        <div v-else-if="step === 'setting'" class="blast-panel">
          ...
          <div style="margin-top:12px; display:grid; grid-template-columns: 1fr 1fr; gap:12px;">
            <div>
              <div class="blast-subtitle" style="margin:0 0 6px 0;">Delay min (ms)</div>
              <input
                v-model="delayMinMs"
                type="number"
                min="0"
                style="width:100%; border-radius:12px; padding:10px; border:1px solid rgba(255,255,255,0.10); background: rgba(2,6,23,0.6); color:#e5e7eb;"
              />
            </div>

            <div>
              <div class="blast-subtitle" style="margin:0 0 6px 0;">Delay max (ms)</div>
              <input
                v-model="delayMaxMs"
                type="number"
                min="0"
          ...
          <div style="margin-top:12px; display:flex; gap:12px; flex-wrap:wrap; align-items:center;">
            <label class="blast-subtitle" style="margin:0; display:flex; gap:8px; align-items:center;">
              <input type="checkbox" v-model="addCountry" />
              Auto tambah kode negara (jika nomor diawali 0)
            </label>

            <div v-if="addCountry" style="display:flex; gap:8px; align-items:center;">
              <div class="blast-subtitle" style="margin:0;">Kode negara</div>
              <input
                v-model="countryCode"
                type="text"
                style="width:80px; border-radius:12px; padding:8px 10px; border:1px solid rgba(255,255,255,0.10); background: rgba(2,6,23,0.6); color:#e5e7eb;"
                placeholder="62"
              />
            </div>
          ...
          <div v-if="sendError" class="blast-subtitle" style="margin-top:10px; color:#fca5a5;">
            {{ sendError }}
          </div>

          <div style="margin-top:12px; display:flex; justify-content:space-between; gap:10px;">
            <button type="button" class="btn" @click="setStep('template')">Back</button>
            <button type="button" class="btn-primary" @click="goToSend">Next</button>
          </div>
        </div>
        ...
        <div v-else-if="step === 'send'" class="blast-panel">
          <div style="display:flex; justify-content:space-between; align-items:center; gap:12px; flex-wrap:wrap;">
            <div>
              ...
              <div class="blast-subtitle" style="margin:0;">
                Total: {{ contacts.length }} | Sent: {{ progressStats.sent }} | Failed: {{ progressStats.failed }}
              </div>
            </div>
          ...
          <div style="margin-top:12px; overflow:auto; max-height:280px; border:1px solid rgba(255,255,255,0.08); border-radius:12px;">
            <table style="width:100%; border-collapse:collapse; font-size:13px;">
              <thead>
                <tr>
                  <th style="text-align:left; padding:10px; border-bottom:1px solid rgba(255,255,255,0.08); width:70px;">#</th>
                  <th style="text-align:left; padding:10px; border-bottom:1px solid rgba(255,255,255,0.08);">name</th>
                  <th style="text-align:left; padding:10px; border-bottom:1px solid rgba(255,255,255,0.08);">no_wa</th>
                  <th style="text-align:left; padding:10px; border-bottom:1px solid rgba(255,255,255,0.08); width:90px;">status</th>
                  <th style="text-align:left; padding:10px; border-bottom:1px solid rgba(255,255,255,0.08);">error</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(l, idx) in logs" :key="idx">
                  <td style="padding:10px; border-bottom:1px solid rgba(255,255,255,0.06);">{{ l.index }}</td>
                  <td style="padding:10px; border-bottom:1px solid rgba(255,255,255,0.06);">{{ l.name }}</td>
                  <td style="padding:10px; border-bottom:1px solid rgba(255,255,255,0.06);">{{ l.no_wa }}</td>
                  <td style="padding:10px; border-bottom:1px solid rgba(255,255,255,0.06);">{{ l.status }}</td>
                  <td style="padding:10px; border-bottom:1px solid rgba(255,255,255,0.06);">{{ l.error }}</td>
                </tr>
                <tr v-if="!logs.length">
                  <td colspan="5" style="padding:12px; opacity:0.75;">Belum ada progress.</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div style="margin-top:12px; display:flex; justify-content:space-between; gap:10px;">
            <button type="button" class="btn" :disabled="isSending" @click="setStep('setting')">Back</button>
            <button type="button" class="btn" :disabled="isSending" @click="resetAll">Reset</button>
          </div>
        </div>
      </div>
    </div>
...
const ChatbotView = {
  ...
  template: `
    <div class="dash-body">
      <div style="max-width:720px;">
        <div style="font-size:18px; font-weight:700;">CHATBOT</div>
        <div class="small-status" style="margin-top:10px;">
          Placeholder. Next development: list chats, rules, auto-reply, schedule.
        </div>
      </div>
    </div>
  `,
